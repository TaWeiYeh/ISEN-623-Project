...
class DynamicalSystem:
	...
	def set_final_cost(self, Q_f):
		self.Q_f = Q_f
	def calculate_cost(self, x, u, **kwargs):
		if 'casadi' in kwargs:
			return 0.5*(x-self.goal).T@self.Q@(x-self.goal) + u.T@self.R@u
		else:
			return 0.5*((x-self.goal).T.dot(self.Q).dot(x-self.goal) + u.T.dot(self.R).dot(u))
	def calculate_final_cost(self, x, **kwargs):
		if 'casadi' in kwargs:
			return 0.5*(x-self.goal).T@self.Q_f@(x-self.goal)
		else:
...
class Car(DynamicalSystem):
	def __init__(self):
		...
		self.goal = np.zeros(4)
	def runge_kutta_integration(self, f, x, u, **kwargs):
		k1 = f(x, u, **kwargs)
		k2 = f(x + 0.5 * self.dt * k1, u, **kwargs)
		k3 = f(x + 0.5 * self.dt * k2, u, **kwargs)
		k4 = f(x + self.dt * k3, u, **kwargs)
		x_next = x + self.dt / 6 * (k1 + 2 * k2 + 2 * k3 + k4)
		return x_next
	def dynamics(self, x, u, **kwargs):
		if 'casadi' in kwargs:
			dxdt = [0, 0, 0, 0]
			dxdt[0] = x[3] * ca.sin(x[2])
			dxdt[1] = x[3] * ca.cos(x[2])
			dxdt[2] = u[1] * x[3]
			dxdt[3] = u[0]
			return ca.vertcat(*dxdt)
		else:
			dxdt = np.zeros(4)
			dxdt[0] = x[3] * np.sin(x[2])
			dxdt[1] = x[3] * np.cos(x[2])
			dxdt[2] = u[1] * x[3]
			dxdt[3] = u[0]
			return dxdt
	def transition(self, x, u, **kwargs):
		x_next = self.runge_kutta_integration(self.dynamics, x, u, **kwargs)
		# x_next = np.zeros(4)
